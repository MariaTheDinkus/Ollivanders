...
public class LatheBlockEntity extends BlockEntity {
	...
	private void tick() {
		...
		if (hasPlayer.get() && !stack.isEmpty() && match.isPresent()) {
			if (progress < match.get().getCookTime()) {
				prevProgress = progress;
				progress++;
			} else {
				if (!world.isClient()) {
					match.ifPresent(recipe -> craftedStack.set(recipe.craft(inventory, world.getRegistryManager())));
					int i = MathHelper.floor(match.get().getExperience());
					float f = MathHelper.fractionalPart(match.get().getExperience());
