...
public class SpellProjectileEntity extends PersistentProjectileEntity {
    ...
    public void tick() {
        ...
        while (!isRemoved()) {
            ...
            if (entityHitResult == null || getPierceLevel() <= 0) break;
            hitResult = null;
        }
    ...
    protected void onBlockHit(BlockHitResult blockHitResult) {
		...
		if (!world.isClient()) {
            ActionResult result = spell.onHitBlock(world, blockHitResult);
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (!world.isClient()) {
            ActionResult result = spell.onHitEntity(world, entityHitResult);
            if (result == ActionResult.SUCCESS || result == ActionResult.FAIL) {
                discard();
            }
        }
